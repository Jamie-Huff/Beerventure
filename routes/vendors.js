...
module.exports = (db) => {
  router.get("/", (req, res) => {
    ...
    let query = `
    SELECT items.id, price, items.name, category, vendors.city as city, date_posted, image, vendors.name as vendor_name
    FROM items
  ...
  router.get("/searchByCity", (req, res) => {
    ...
    let query = `
    SELECT price, items.id, items.name, category, vendors.city as city, date_posted, image, vendors.name as vendor_name
    FROM items
    JOIN vendors ON vendors.id = vendor_id
    WHERE city = $1
    `
